...
export default function CAMViewer({ jobId }) {
  ...
  if (!jobId) {
    return (
      <div className="glass" style={{ borderRadius: 20, padding: "60px 40px", textAlign: "center", color: "#64748B" }}>
        <div style={{ fontSize: 56, marginBottom: 16 }}>📋</div>
        <p style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>No analysis loaded</p>
        <p style={{ fontSize: 13 }}>Upload documents and run the pipeline first.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }} className="fade-in-up">
      ...
      <div className="glass" style={{
        ...
        border: "2px solid rgba(54,204,235,.25)",
      }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "center", marginBottom: 28 }}>
          {[["📊","Five Cs Analysis"],["📈","SHAP Waterfall"],["🔒","Fraud Score"],["👤","Promoter Profile"],["⚖️","Final Verdict"],["🏦","Sector Analysis"],["📉","Financial Ratios"],["🔍","Risk Narrative"]].map(([icon,label]) => (
            <span key={label} style={{ padding: "5px 14px", borderRadius: 20, background: "rgba(8,145,178,.1)", color: "#0891b2", fontSize: 11, fontWeight: 600 }}>
              {icon} {label}
            </span>
          ))}
        </div>
        ...
        <div style={{ marginBottom: 24 }}>
          {checking && (
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 20px", borderRadius: 20, background: "rgba(148,163,184,.1)", color: "#64748B", fontSize: 13 }}>
          ...
          {pdfOk === false && (
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 20px", borderRadius: 20, background: "rgba(239,68,68,.1)", color: "#EF4444", fontSize: 13, fontWeight: 600 }}>
              ✗ PDF not ready — run the pipeline first
            </div>
          )}
          {ready && (
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 20px", borderRadius: 20, background: "rgba(16,185,129,.1)", color: "#059669", fontSize: 13, fontWeight: 600 }}>
        ...
        {ready ? (
          ...
          <button disabled style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "16px 40px", borderRadius: 16, background: "#CBD5E1", color: "#94A3B8", fontWeight: 800, fontSize: 16, border: "none", cursor: "not-allowed" }}>
            ⬇ Download CAM Report (PDF)
          </button>
        )}
        ...
        {ready && (
          <div style={{ marginTop: 14 }}>
            <button
              ...
              style={{ background: "none", border: "none", color: "#0891b2", fontSize: 12, cursor: "pointer", textDecoration: "underline", fontWeight: 600 }}
            >
      ...
      {showViewer && ready && (
        <div className="glass" style={{ borderRadius: 20, overflow: "hidden" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", borderBottom: "1px solid rgba(54,204,235,.15)" }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: "#0891b2" }}>📋 CAM · {jobId.slice(0, 8).toUpperCase()}</span>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => window.open(pdfAbsUrl, "_blank", "noopener,noreferrer")} style={{ background: "rgba(8,145,178,.1)", border: "none", borderRadius: 8, padding: "4px 10px", cursor: "pointer", fontSize: 11, color: "#0891b2", fontWeight: 600 }}>
                ↗ Full screen
              </button>
              <button onClick={() => setShowViewer(false)} style={{ background: "rgba(239,68,68,.1)", border: "none", borderRadius: 8, padding: "4px 10px", cursor: "pointer", fontSize: 12, color: "#EF4444", fontWeight: 700 }}>
                ✕
              </button>
            </div>
          </div>
